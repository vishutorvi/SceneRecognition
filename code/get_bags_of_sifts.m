...
image_feats = zeros(len,vocab_size);
for i=1:len
    [locations, SIFT_features] = vl_dsift(single(imread(image_paths{i,1})),'step',10,'fast');
    D = vl_alldist2(single(SIFT_features),vocab1);
    D = D ./ size(SIFT_features,2);
    row = size(D,1);
    for j=1:row
        [Y,I] = min(D(j,:));
        image_feats(i,I) = image_feats(i,I) + 1;
    end
end