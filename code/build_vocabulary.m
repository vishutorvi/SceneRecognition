...
len = length(image_paths);
bag_of_sift_features = 0;
for i=1:len
[locations, SIFT_features] = vl_dsift(single(imread(image_paths{i,1})),'step',50,'fast'); 
if bag_of_sift_features == 0
    bag_of_sift_features = SIFT_features;
else
   bag_of_sift_features = horzcat(bag_of_sift_features,SIFT_features); 
end

end
[centers, assignments] = vl_kmeans(single(bag_of_sift_features), vocab_size);
vocab = centers';
